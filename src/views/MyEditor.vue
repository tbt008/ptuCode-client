  ...
  <div class="addBlog">
    ...
    <el-form ref="formRef" label-width="100px">
      <el-radio-group class="editorType" v-model="editorType" label="size control">
        <el-radio-button value="1">文章</el-radio-button>
        <el-radio-button value="2">随笔</el-radio-button>
        <el-radio-button value="3">待扩展</el-radio-button>
      </el-radio-group>
      <div v-if="editorType != 2">
        ...
        <el-form-item label="文章封面">
          <el-upload
            ...
            action="https://www.tbt-blog.cn/tbt/file/addFile"
...
editorConfig.MENU_CONF['uploadImage'] = {
  async customUpload(file, insertFn) {
    ...
    axios.post('https://www.tbt-blog.cn/tbt/file/addFile', data).then((res) => {
      const url = res.data
      const alt = '图片无法显示'
      const href = url
...
onMounted(() => {
  // 查询所有分类
  axios.get(`https://www.tbt-blog.cn/tbt/type/getAll`).then((res) => {
    options.value = res.data.data
    // 回显
    if (router.currentRoute.value.query.id) {
      const id = router.currentRoute.value.query.id
      axios.get(`https://www.tbt-blog.cn/tbt/article/${id}`).then((res) => {
...
const imgAdd = (pos: any, $file: any) => {
  ...
  axios.post('https://www.tbt-blog.cn/tbt/file/addFile', formData).then((res) => {
    const url = res.data
    // pos = 1
...
function primary() {
  if (editorType.value == '1') {
    ...
    if (router.currentRoute.value.query.id) {
      ...
      axios.post('https://www.tbt-blog.cn/tbt/article/update', data)
      ...
      data.append('label', dynamicTags.value)
      data.append('type', type.value)
      axios.post('https://www.tbt-blog.cn/tbt/article/upload', data)
      ElMessage.success('发布成功！！！')
    }
    ...
    axios.post(`https://www.tbt-blog.cn/tbt/Live/add`, data)
    ElMessage.success('发布成功！！！')
  }
}
function cancel() {
